 ...
 *
 * (see
 * https://stackoverflow.com/questions/30345665/how-to-customize-logging-for
...
@Pattern(
	name = "Builder",
	participants = { 
		"Builder", 			// The LogBuilder itself (this class)
		"ConcreteBuilder", 	// Also the LogBuilder
		"Director", 		// All classes that use the LogBuilder
		"Product", 			// The string that is finally created by the LogBuilder
	}
)
public class LogBuilder {
	...
	protected static LogBuilder doCreateMessage(String level) {
		...
		if (session != null) {
			...
			Client client = UserManager.getInstance().getClientById(
					session.getClientId());
			if (client != null) {
